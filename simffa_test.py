...
def simffa_facegen_reextract_neural(dbname_old, dbname_new):
	...
	for key in exp_keys:
		...
		new_id = new_ids[0]
		new_trials.insert_trial_docs(sfexp.reExtract(job_old, new_bandit, new_id, new_trials))
...
#  behavioural tasks on facegen dataset with fsi measured on fbo dataset
def simffa_facegen_task_withFSI(dbname='simffa_facegen', randomSearch=False):
	
	host = 'localhost'
	port = 22334
	nExps = 3 

	bandit_names = ['simffa.simffa_bandit_facegens.Simffa_FaceGenTasksFSI_Bandit_V1',
					'simffa.simffa_bandit_facegens.Simffa_FaceGenTasksFSI_Bandit_L2',
					'simffa.simffa_bandit_facegens.Simffa_FaceGenTasksFSI_Bandit_L3']
	bandit_args_list = [() for _i in range(nExps)]
	bandit_kwargs_list = [{} for _i in range(nExps)]
	exp_keys = ['simffa_facegen_V1', 'simffa_facegen_L2', 'simffa_facegen_L3']
